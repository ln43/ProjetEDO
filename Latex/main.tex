...
Dans le cas KPP et monostable on a l'existence d'un continuum de vitesses c tel que le probleme (1) a une solution, et donc une infinite de solutions (U,c); cq de (1). Plus precisesement, pour le cas KPP et monostable, il existe un vitesse minimale c$_0$ unique telle que quelque soit $c > c_0$ l'equation (1) a une
solution c (unique a translation pres).


    Donc il existe une vitesse minimale $c_0$ telle que  $\forall c>=c_0, \exists!U(z)$ monotone, solution de l'EDO avec les conditions aux bords $\lim_{z \to +\infty} U(z)=u_+$ et $\lim_{z \to -\infty} U(z)=u_-$. \\
Ces  ondes  progressives  représentent  l’invasion  de l’état  0  par l’état  1  et  consistent  en
la propagation d’un profil constant à vitesse c.

    De plus comme l'équation est monostable avec aucun équilibre entre $u_+$ et $u_-$, si on prend comme condition initiale l'équilibre instable $u_+$ avec une perturbation à support borné, alors la solution tend vers deux fronts de vitesse minimale $c_0$. \\
    
    D'autre part comme l'équation est monostable avec aucun équilibre entre $u_+$ et $u_- $ et si $ f^\prime(u)< f^\prime(u_+)$ pour un voisinnage de $u_+$ alors : \\
    
    $c_0=2 \sqrt{f^\prime(u_+)}= 2 \sqrt{\alpha - 2 \frac{\alpha}{K} \times 0} = 2 \sqrt{\alpha}$
 
Ce problème possède une solution si et seulement si $c\geqslant c_0$.
    
    On peut donc déterminer la vitesse de cette propagation. Il y a ici, cependant, une question importante. Nous avons démontré l’existence d’onde pour toutes les vitesses supérieures ou égales à la vitesse minimale $c_0$. Quelle est donc la vitesse de propagation
qu’on observe dans notre application, est-ce $c_0$ ou une autre vitesse plus grande ?
